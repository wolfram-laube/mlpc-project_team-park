...
window_size = 3.0  # Window size in seconds
...
def detect_command_pairs(audio_path, model, sr=16000, window_size=1.5, step_size=0.2, threshold=0.8):
    y, _ = librosa.load(audio_path, sr=sr)
    window_samples = int(window_size * sr)
    step_samples = int(step_size * sr)

    windows = librosa.util.frame(y, frame_length=window_samples, hop_length=step_samples).T
    ...
    for start in boundaries:
        timestamp = start * step_samples / sr
...
with open(output_csv, 'w', newline='') as csvfile:
    ...
    for wav_file in tqdm(selected_files, desc="Processing files"):
        ...
        command_segments = detect_command_pairs(file_path, model, sr=sample_rate, window_size=window_size,
